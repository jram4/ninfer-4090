...
Modified by satellitedown for Cinference: replace machine-specific interpreter instructions.
...
Use a Python 3.11 interpreter selected for your environment, for example `python3.11` or
the Python executable in a project virtual environment. Do not assume the shell's `python3`
selects that version. Normal resources are `build/`, `out/qwen3_6_27b.ninfer`, its `.conversion.json`
report, and `profiles/ncu/`, `profiles/nsys/`, `profiles/bench/`. CUDA 13.1 is the upstream-validated
development toolkit.
