...
Speculative decoding is disabled by default. Select MTP with one to ten draft positions, or the
...
Only one speculative backend can be enabled per Engine. The upstream NInfer [performance results](performance.md)
