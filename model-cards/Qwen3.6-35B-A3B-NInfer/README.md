...
In Cinference, `--draft-tokens` accepts `1..10` for MTP and `1..15` for DFlash. The DFlash value `7` is the
...
> Modified by satellitedown for Cinference: the MTP ranges on this page apply to this fork.
> This upstream NInfer artifact card and its measurements are retained for reference;
> its performance and evaluation tables are not new Cinference measurements.

The artifact supports:

- text generation in thinking and non-thinking modes;
- image, multi-image, video, and mixed multimodal messages;
- MTP speculative decoding with draft windows from one to ten in Cinference;
