// Modified by satellitedown for Cinference: cover the extended MTP draft-token option range.
    ...
    failures += expect(parsed.speculative.draft_tokens == 5, "MTP window");
    {
        const auto widest =
            parse_for_test({"ninfer_bench", "--weights", "model.ninfer", "--spec", "mtp",
                            "--draft-tokens", "10", "--lm-head-draft"});
        failures += expect(widest.speculative.draft_tokens == 10, "maximum MTP window");
    }
