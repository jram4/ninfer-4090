...
> Modified by satellitedown for Cinference: portable interpreter and local-data instructions.
> Retained capability scores are upstream NInfer measurements, not new Cinference results.
...
Before running a Needle-in-a-Haystack suite, set every `local_path` and `tokenizer_path` in
that YAML to your local dataset snapshot and Qwen3.6-35B-A3B tokenizer directory. The checked-in
`/path/to/...` values are placeholders, not download locations.
...
The download prints your actual dataset snapshot directory. Substitute it for the YAML's
`local_path` placeholders and set `tokenizer_path` to your downloaded tokenizer before planning
or running the suites.
