...
void ProgramImpl::prepare_graphs() {
    ...
    if (speculative_backend == SpeculativeBackend::Mtp) {
        ...
        // The frontier buckets cross engine-internal route boundaries that change the captured node
        // topology. Those boundaries are not enumerable from the plan, so the class is derived from
        // the capture itself: profiles with the same captured signature share one executable.
        std::vector<std::uint64_t> route_signatures;
        for (std::uint32_t batch_size = 1; batch_size <= max_concurrency; ++batch_size) {
            for (const GraphExecutionProfile planned : planned_profiles) {
                ...
                execution::capture_mtp_decode_batch(
                    ...
                    profile.definition);
                const std::uint64_t signature = profile.definition.topology_signature().second;
                const auto existing =
                    std::find(route_signatures.begin(), route_signatures.end(), signature);
                const std::uint32_t route =
                    existing != route_signatures.end()
                        ? static_cast<std::uint32_t>(existing - route_signatures.begin())
                        : (route_signatures.push_back(signature),
                           static_cast<std::uint32_t>(route_signatures.size() - 1));
                profile.topology_class = route * max_concurrency + (batch_size - 1U);
            }
        }
    }
