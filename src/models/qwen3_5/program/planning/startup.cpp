...
void validate_target_options(const execution::Parameters& parameters, DeviceContext& device,
    ...
    case SpeculativeBackend::Mtp:
        if (options.speculative.draft_tokens == 0 ||
            ...
            throw std::invalid_argument("MTP draft window must be in [1,10]");
        }
        break;
...
std::unique_ptr<SequencePlanImpl> build_sequence_candidate(const SequencePlanningInputs& inputs,
    ...
    if (impl->use_cuda_graph) {
        ...
        if (impl->speculative_backend == SpeculativeBackend::None) {
            ...
            // Route families are discovered from the capture itself (see the MTP capture loop).
            // Windows wider than six positions observe up to three of them (measured at K=10, batch
            // 1); narrower windows observe one. This is a reservation bound, not an exact count.
            const std::size_t families = impl->draft_window + 1U > 6U ? 3ULL : 1ULL;
            const std::size_t per_batch_allowance = graph_topology_allowance(
                ...
                [&](GraphExecutionProfile profile) {
                    ...
                    return families * (final_visible <= 4096 ? 12ULL : 82ULL) * kMiB;
                },
                "MTP graph allowance");
