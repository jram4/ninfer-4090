...
// Single-pass small-T Q5 GEMM for Ada: out^T[T x N] = x^T[T x K] * W^T[K x N] on
// mma.m16n8k16 with tokens as M (zero-padded to 16) and eight weight rows as N.
//
// A CTA owns 8 * RowTiles rows over the whole K and streams them through a cp.async pipeline of
// GroupsPerStage 64-weight groups per stage. Global reads are contiguous 16-byte chunks per row;
// the row paddings below keep the fragment reads out of shared memory bank-conflict free. x is
// L2-resident at these widths and is prefetched into registers one stage ahead instead of taking
// pipeline space. Warps split each stage's groups and reduce through shared memory at the end, so
// every weight byte is read once for any T <= 8 * TokenTiles.
//
// Per group a lane owns 16 consecutive weights (8 code bytes, 16 high bits) of one row. MMA step s
// maps K slots {2q, 2q+1} to weights 16q + 4s + {0,1} and slots {2q+8, 2q+9} to 16q + 4s + {2,3};
// x is read with the same permutation, so the dot product is unchanged. Codes decode to exact bf16
// integers and the fp16 group scale is applied in fp32 after the group's four MMAs.
template <int RowTiles, int Warps, int TokenTiles, int GroupsPerStage, int Stages>
struct Q5AdaSmallTSchedule {
    ...
    static constexpr int kHighStride  = 8 * kG + 16;
...
// Two codes of one byte plus their inverted high bits (bits 0 and 1) -> exact bf16x2 integers.
...
// Epilogue receives (row, token, fp32 dot product) for every row of the CTA and token < tokens.
template <class S, int K, class Epilogue>
__launch_bounds__(S::kThreads) __global__ void q5_ada_small_t_mma_kernel(
    ...
    static_assert(K % (Q5RowSplitStorage::kGroupK * S::kG) == 0);
    constexpr int kGroups     = K / Q5RowSplitStorage::kGroupK;
    ...
    const int row_base = static_cast<int>(blockIdx.x) * kRows;

    const std::uint8_t* code_src =
        codes + static_cast<std::int64_t>(row_base) * kGroups * Q5RowSplitStorage::kCodeBytesPerGroup;
    const std::uint8_t* high_src =
        high + static_cast<std::int64_t>(row_base) * kGroups * Q5RowSplitStorage::kHighBytesPerGroup;
    const std::uint8_t* scale_src =
        scales +
        static_cast<std::int64_t>(row_base) * kGroups * Q5RowSplitStorage::kScaleBytesPerGroup;
    ...
    const auto issue = [&](int stage) {
        ...
        for (int i = tid; i < kRows * kCodeChunks; i += S::kThreads) {
            ...
            cp_async<16, Cache::cg>(code_dst + row * S::kCodeStride + 16 * c,
                                    code_src + (static_cast<std::int64_t>(row) * kGroups + g0) * 32 +
                                        16 * c);
        }
        ...
        for (int i = tid; i < kRows * kHighChunks; i += S::kThreads) {
            ...
            cp_async<16, Cache::cg>(high_dst + row * S::kHighStride + 16 * c,
                                    high_src + (static_cast<std::int64_t>(row) * kGroups + g0) * 8 +
                                        16 * c);
        }
        if constexpr (S::kG == 4) {
            for (int row = tid; row < kRows; row += S::kThreads) {
                cp_async<8>(scale_dst + row * S::kScaleStride,
                            scale_src + (static_cast<std::int64_t>(row) * kGroups + g0) * 2);
            }
        } else {
            ...
            for (int i = tid; i < kRows * kScaleChunks; i += S::kThreads) {
                ...
                cp_async<16, Cache::cg>(
                    scale_dst + row * S::kScaleStride + 16 * c,
                    scale_src + (static_cast<std::int64_t>(row) * kGroups + g0) * 2 + 16 * c);
            }
        }
    };
...
#pragma unroll
        for (int j = 0; j < J; ++j) {
            const int k0 = (stage * S::kG + warp + j * S::kWarps) * Q5RowSplitStorage::kGroupK;
...
#pragma unroll
            for (int r = 0; r < R; ++r) {
                ...
                const unsigned inv = ~(hw >> (16 * (q & 1)));
                float d0 = 0.0f, d1 = 0.0f, d2 = 0.0f, d3 = 0.0f;
#pragma unroll
                for (int s = 0; s < 4; ++s) {
                    const unsigned w     = s < 2 ? word.x : word.y;
                    const unsigned shift = 16u * static_cast<unsigned>(s & 1);
                    const unsigned b0 = q5_ada_decode_pair((w >> shift) & 0xffu, inv >> (4 * s));
                    const unsigned b1 =
                        q5_ada_decode_pair((w >> (shift + 8u)) & 0xffu, inv >> (4 * s + 2));
...
#pragma unroll
        ...
        epilogue(row_base + row, token, sum);
    }
}

// Launches one CTA per 8 * RowTiles rows; rows must be a multiple of that.
template <class S, int K, class Epilogue>
void q5_ada_small_t_mma_launch(const __nv_bfloat16* x, std::int32_t x_ld,
                               const std::uint8_t* codes, const std::uint8_t* high,
                               const std::uint8_t* scales, std::int32_t rows, std::int32_t tokens,
                               Epilogue epilogue, cudaStream_t stream) {
    static const bool configured = [] {
        CUDA_CHECK(cudaFuncSetAttribute(q5_ada_small_t_mma_kernel<S, K, Epilogue>,
                                        cudaFuncAttributeMaxDynamicSharedMemorySize,
                                        S::kSmemBytes));
        return true;
    }();
    (void)configured;
    q5_ada_small_t_mma_kernel<S, K, Epilogue>
